...
import com.apecircle.easytranslib.bean.CusRow;
import com.apecircle.easytranslib.bean.FloderBean;
...
public class WriteXlsAndTransManager extends BaseSheet {
    ...
    private List<String> mSrcList = new ArrayList<>(); //用来default中的value值
    private Workbook mWorkbook;
    private GoogleApi googleApi = null;

    private static class Holder {
        static WriteXlsAndTransManager writeXlsManager = new WriteXlsAndTransManager();
    }

    public static WriteXlsAndTransManager getInstance() {
        return Holder.writeXlsManager;
    }

    private WriteXlsAndTransManager() {
        ...
        if (rootFile.exists()) {
            ...
            if (bean != null) {
                ...
                if (!bean.floderPaths.isEmpty()) {
                    ...
                    for (int i = 0; i < 1; i++) { //浏览文件夹
                        String floderPath = bean.floderPaths.get(0);
                            ...
                            for (String valueName : valueNames) { //浏览各个文件夹中的文件
                                ...
                                for (int langIndex = 0; langIndex < bean.languages.size(); langIndex++) {
                                    ...
                                    if (lists != null && !lists.isEmpty()) {
                                        writeDataToXls(lists, sheet, langIndex, String.valueOf(bean.languagesNumber.get(langIndex)));
                                    }
                                }
                            }
                        }
    ...
    private FloderBean getFloderBean(File rootFile) {
        ...
        bean.languagesCode = new ArrayList<>();
        bean.languagesNumber = new ArrayList<>();
        ...
        if (rootFile.exists()) {
            ...
            for (File file : files) {
                if (file.isDirectory()) {
                    ...
                    bean.languagesCode.add(SheetEnum.getLanByFolderName(getFloderName(file)).getCode());
                    bean.languagesNumber.add(SheetEnum.getLanByFolderName(getFloderName(file)).getNumber());
     ...
     * @param rIndex
     */
    private void writeDataToXls(List<CusRow> lists, Sheet sheet, int rIndex, String code) {
                    ...
                    if (index != -1) {
                        ...
                        try {
                            if ("36".equals(code)) {
                                ...
                                value = googleApi.translate(src, "36", code);
                                Thread.sleep(1000);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        ...
                        System.out.println(cusRow.key + "------" + src + "(" + "36" + ")" + "------" + value + "(" + code + ")");
                    }
